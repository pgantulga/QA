...
export class NotificationService {
  ...
  getNotifications(user) {
    return this.db
      .collection('notifiers', (ref) =>
        ref
          .where('notifier', '==', user.uid)
  ...
  async getAllNotificationsNumber(user) {
    const notifications = await this.db
      .collection('notifiers', (ref) =>
        ref.where('notifier', '==', user.uid).orderBy('createdAt', 'desc')
      )
      .ref.get();
  ...
  createNotificationObject(
    ...
    return this.notificationObjectsRef
      .add({
        entity: entityId,
      ...
      .then(async (res) => {
        ...
        if (entityType === 0) {
          ...
          messageText = await this.generateMessage(
            user.uid,
            entityType,
            entityId
          );
  ...
  updateNotifier(notifierId, data) {
    return this.notifiersRef.doc(notifierId).set(data, { merge: true });
  }
  ...
  private async generateMessage(actor, entityType, entityId, parent?) {
    ...
    const messageTitle =
      entityType >= 4
        ? await this.getPostTitle(parent)
        : await this.getPostTitle(entityId);
    if (entityType === 1) {
  ...
  getPowerUsers() {
    return this.db
      .collection('users', (ref) => ref.where('roles.moderator', '==', true))
      .get()
      .toPromise();
  }
}
